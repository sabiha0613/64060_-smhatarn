...
fviz_cluster(K5,data=DATA) #visualize the output
```
...
```
  ...
  By the output of > aggregate(DATA,by=list(fit$cluster),FUN=mean), we can observe the following:
  ...
  ```{r}
  #(c)Is there a pattern in the clusters with respect to the numerical variables (10 to 12)? (those not used
  ...
  Cluster 5 with highest Market_Cap,highest ROE, highest ROA,highest Asset_Turnover has equal hold Moderate    
  Buy
